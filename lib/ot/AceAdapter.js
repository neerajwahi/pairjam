...
function drawUserMarkerFn(name, colorClass) {
	return function(html, range, left, top, config) {
		...
		caret = '<div class="ace_cursor ' + colorClass + '" style="left: ' + leftPos + 'px; top: ' + top + 'px;'
...
AceAdapter.prototype = {
	...
	onRender: function(fn) {
		...
		var onBeforeRender = (function() {
			...
			if(firstRow !== this.lastFirstRow || lastRow != this.lastLastRow) {
	...
	updateMarkers: function(names, colorClasses, cursors) {
		var markers = this.session.getMarkers(true), marker1, marker2, id, cursor;
		for(id in cursors) {
			if(!this.markerIds.hasOwnProperty(id)) {
				// Add marker
				this.addMarker(id, names[id], colorClasses[id], cursors[id]);
			}
		}
